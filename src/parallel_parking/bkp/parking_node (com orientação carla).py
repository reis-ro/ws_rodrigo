...
class ParallelParkingNode(Node):
    ...
    def veh_control(self, initial_pos, transition_pos, goal_pos, update_pos):
        global flag, case
        self.reverse = False
        # angle = np.arcsin(wheel_base/R_min/np.sqrt(3))/0.8*180/np.pi
        angle = 70
        steering = angle/70
        if update_pos[1] < launch_coord[1]:  # Move backward until start_pos
            ...
            self.brake = 0.0
        elif launch_coord[1] <= update_pos[1] < transition_pos[1]:  # First turn
            case = 2
            ...
            self.brake = 0.0
        elif self.flag == 0 and transition_pos[1] <= update_pos[1] < goal_pos[1]:  # Second turn
            case = 3
            self.steer = -steering
        ...
        elif self.flag == 1:  # After parking is complete, make adjustments
            case = 5
            self.steer = 0.0
    ...
    def veh_mission(self, msg):
        ...
        if True:
            ...
            self.veh_control(launch_coord, transition, goal, self.global_point)
